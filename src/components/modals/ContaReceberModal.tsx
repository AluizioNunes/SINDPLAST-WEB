...
import { createContaReceber, updateContaReceber } from '@/lib/services/contaReceberService';
...
export default function ContaReceberModal({
    ...
    useEffect(() => {
        if (isOpen) {
            getCentroCustos({ limit: 1000 }).then(({ data }) => {
                setCentroCustos(data.map((cc: any) => ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (contaReceber) {
                await updateContaReceber(contaReceber.id, formData);
            } else {
